    ...
    <div class="flex-1 flex flex-col justify-center items-center text-center space-y-8 border-r border-white/5">
        ...
        <div class="relative py-8">
            <div class="flex gap-4">
                @for($i = 0; $i < 8; $i++)
                    <div class="w-10 h-10 rounded-xl border-2 border-white/10 flex items-center justify-center transition-all {{ strlen($otp) > $i ? 'bg-white/10 border-white/20' : '' }}">
                        <span class="text-xl font-black italic">{{ substr($otp, $i, 1) ?: '' }}</span>
                    </div>
                @endfor
            </div>
    ...
    <div class="flex-1 flex items-center justify-center">
        <div class="grid grid-cols-3 gap-2.5 w-full max-w-[320px]">
            @foreach([1,2,3,4,5,6,7,8,9] as $num)
                <button wire:click="addDigit({{ $num }})" 
            ...
            @endforeach
            <button wire:click="clearOtp" class="h-14 rounded-2xl bg-zinc-900 border border-white/5 text-[0.6rem] font-black uppercase opacity-40">Effacer</button>
